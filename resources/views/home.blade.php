...
<div id="imgNews">
    ...
    @endif
    ><a href="/archive/{{ $presses[0]->id }}" class="sepa1" title="{{ $presses[0]->title }}"><div class="imgText">{{ $presses[0]->title }}</div></a></div>
    <div class="imgShow newsIntro2"
    @if (!empty($presses->items()[1]->picture))
    style="background-image: url('{{ $presses[1]->picture }}')"
    @endif
    ><a href="/archive/{{ $presses[1]->id }}" class="newsLink bg" title="{{ $presses[1]->title }}"><div class="imgText">{{ $presses[1]->title }}</div></a></div>
    <div  class="imgShow newsIntro3"
    @if (!empty($presses->items()[2]->picture))
    style="background-image: url('{{ $presses[2]->picture }}')"
    @endif
    ><a href="/archive/{{ $presses[2]->id }}" class="sepa1" title="{{ $presses[2]->title }}"><div class="imgText">{{ $presses[2]->title }}</div></a></div>
    <div class="imgShow quote1" >
    <p style="margin-top:40px;">为天地立心，为生民立命，为往圣继绝学，为万世开太平。
        <span style="float: right;magin-left:20px;">——张载</span>
    </p>
    </div>
    <div class="imgShow quote1" >
        <p style="margin-top:60px;">问，乃思之虔诚。
    ...
    @endif
    ><a href="/archive/{{ $presses[3]->id }}" class="newsLink bg1" title="{{ $presses[3]->title }}"><div class="imgText">{{ $presses[3]->title }}</div></a></div>
    <div class="imgShow newsIntro5"
    @if (!empty($presses->items()[4]->picture))
    style="background-image: url('{{ $presses[4]->picture }}')"
    @endif
    ><a href="/archive/{{ $presses[4]->id }}" class="sepa2" title="{{ $presses[4]->title }}"><div class="imgText">{{ $presses[4]->title }}</div></a></div>
    <div class="imgShow newsIntro6"
    @if (!empty($presses->items()[5]->picture))
    style="background-image: url('{{ $presses[5]->picture }}')"
    @endif
    ><a href="/archive/{{ $presses[5]->id }}" class="newsLink bg2" title="{{ $presses[5]->title }}"><div class="imgText">{{ $presses[5]->title }}</div></a></div>
    
    <div class="imgShow quote2" >
    <img src="/img/xiaoxun.png" alt=""></img></div>
</div>
<div id="manyLinks">
    <div class="links1">
